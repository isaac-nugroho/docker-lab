...
#elif [[ -e ${CASSANDRA_DATA}/previous_address.txt ]]; then
  #  replace self
#  REPLACE_ADDRESS=$(cat ${CASSANDRA_DATA}/previous_address.txt)
#  if [[ ! -z "$REPLACE_ADDRESS" -a "$REPLACE_ADDRESS" != $(hostname -i) ]]; then
#    JVM_OPTS="$JVM_OPTS -Dcassandra.replace_address=$REPLACE_ADDRESS"
#  fi
fi
